...
const githubDbToken = process.env.GITHUB_DB_TOKEN || "";
const githubDbRepo = process.env.GITHUB_DB_REPO || "";
const githubDbBranch = process.env.GITHUB_DB_BRANCH || "data-store";
const githubDbPath = process.env.GITHUB_DB_PATH || "data/db.json";
const useGithubDb = Boolean(githubDbToken && githubDbRepo);
const useSupabase = !useGithubDb && Boolean(supabaseUrl && supabaseServiceRoleKey);
const storeCacheMs = Number(process.env.STORE_CACHE_MS || 3000);
let storeCache = null;
...
async function loadDb() {
  if (useGithubDb) return loadGithubDb();
  await ensureDb();
...
async function saveDb(db) {
  setStoreCache(db);
  if (useGithubDb) return saveGithubDb(db);
  await writeFile(dbPath, `${JSON.stringify(db, null, 2)}\n`);
}

function setStoreCache(db) {
  storeCache = { db: normalizeDb(db), loadedAt: Date.now() };
}
...
async function githubContentsRequest(method, body) {
  const response = await fetch(`https://api.github.com/repos/${githubDbRepo}/contents/${encodeURIComponent(githubDbPath).replaceAll("%2F", "/")}${method === "GET" ? `?ref=${encodeURIComponent(githubDbBranch)}` : ""}`, {
    method,
    headers: {
      accept: "application/vnd.github+json",
      authorization: `Bearer ${githubDbToken}`,
  ...
  if (!response.ok) {
    const error = new Error(data?.message || "GitHub 数据存储请求失败");
...
async function readGithubDbFile() {
  const file = await githubContentsRequest("GET");
  const content = String(file.content || "").replace(/\s/g, "");
  return {
    sha: file.sha,
    db: normalizeDb(JSON.parse(Buffer.from(content, "base64").toString("utf8")))
  };
}

async function loadGithubDb() {
  try {
    const { db } = await readGithubDbFile();
    setStoreCache(db);
    return db;
  } catch (err) {
    if (err.status === 404) {
...
async function saveGithubDb(db) {
  let sha = "";
  try {
    ({ sha } = await readGithubDbFile());
  } catch (err) {
    if (err.status !== 404) throw err;
  }
  try {
    await githubContentsRequest("PUT", {
      message: `Update OvO data ${new Date().toISOString()}`,
      content: Buffer.from(`${JSON.stringify(normalizeDb(db), null, 2)}\n`).toString("base64"),
      branch: githubDbBranch,
      ...(sha ? { sha } : {})
    });
  } catch (err) {
    if (err.status === 409) {
      const conflict = new Error("数据刚刚被别人更新了，请重试一次。");
      conflict.status = 409;
      throw conflict;
    }
    throw err;
...
function toAppSquareCommentLike(row) {
  return {
    id: row.id,
...
function toDbSquareCommentLike(like) {
  return {
    id: like.id,
...
async function supabaseOptionalRequest(path) {
  try {
    ...
    if (isMissingSupabaseSchema(err)) return [];
    throw err;
  }
}

function isMissingSupabaseSchema(err) {
  return err?.status === 404 || /schema cache|relation|column|could not find the table/i.test(err?.message || "");
}

function missingSquareSchemaError() {
  const error = new Error("广场数据库还没有初始化。请先在 Supabase SQL Editor 运行 supabase/schema.sql 里的新增表 SQL。");
  error.status = 503;
  return error;
}

async function loadStore() {
  if (storeCache && Date.now() - storeCache.loadedAt < storeCacheMs) return storeCache.db;
  if (!useSupabase) return loadDb();
  const [inboxes, letters, squarePosts, squareLikes, squareComments, squareCommentLikes] = await Promise.all([
    ...
    supabaseOptionalRequest("square_comment_likes?select=*")
  ]);
  const db = {
    ...
    squareCommentLikes: squareCommentLikes.map(toAppSquareCommentLike)
  };
  setStoreCache(db);
  return db;
}
...
async function insertSquarePost(post) {
  ...
  let row;
  try {
    [row] = await supabaseRequest("square_posts", { method: "POST", body: toDbSquarePost(post) });
  } catch (err) {
    if (isMissingSupabaseSchema(err)) throw missingSquareSchemaError();
    throw err;
  }
...
async function insertSquareLike(like) {
  ...
  let row;
  try {
    [row] = await supabaseRequest("square_post_likes", { method: "POST", body: toDbSquareLike(like) });
  } catch (err) {
    if (isMissingSupabaseSchema(err)) throw missingSquareSchemaError();
    throw err;
  }
...
async function insertSquareCommentLike(like) {
  if (!useSupabase) return null;
  let row;
  try {
    [row] = await supabaseRequest("square_comment_likes", { method: "POST", body: toDbSquareCommentLike(like) });
  } catch (err) {
    if (isMissingSupabaseSchema(err)) throw missingSquareSchemaError();
    throw err;
  }
  return toAppSquareCommentLike(row);
}
...
async function insertSquareComment(comment) {
  ...
  let row;
  try {
    [row] = await supabaseRequest("square_comments", { method: "POST", body: toDbSquareComment(comment) });
  } catch (err) {
    if (isMissingSupabaseSchema(err)) throw missingSquareSchemaError();
    throw err;
  }
...
function publicSquareComment(comment, db = { squareCommentLikes: [] }) {
  return {
    id: comment.id,
    ...
    likeCount: db.squareCommentLikes.filter((like) => like.commentId === comment.id).length
  };
}
...
function ownerStats(letters) {
  return {
    ...
    privateReplies: letters.filter((letter) => letter.reply && letter.status !== "replied").length
  };
}

function ownerSentLetters(inbox, db) {
  return db.letters
...
function ownerSquarePosts(inbox, db) {
  return db.squarePosts
    .filter((post) => post.inboxId === inbox.id)
    .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
...
export async function handleApi(req, res, url) {
  ...
  if (req.method === "GET" && path === "/api/health") {
    return sendJson(res, 200, { ok: true, storage: useGithubDb ? "github" : (useSupabase ? "supabase" : "json") });
  }
  ...
  if (req.method === "GET" && path === "/api/square/posts") {
    ...
    const searchValue = cleanText(url.searchParams.get("q"), 80).toLowerCase();
    const posts = db.squarePosts
      .map((post) => publicSquarePost(post, db))
      .filter((post) => {
        if (!searchValue) return true;
        if (searchType === "author") return `${post.author.penName} ${post.author.handle}`.toLowerCase().includes(searchValue);
        if (searchType === "date") return (post.createdAt || "").slice(0, 10) === searchValue;
  ...
  if (req.method === "POST" && path === "/api/square/posts") {
    ...
    if (useSupabase) clearStoreCache();
    if (!useSupabase) {
  ...
  if (req.method === "GET" && squarePostMatch) {
    ...
    const comments = db.squareComments
      ...
      .map((comment) => publicSquareComment(comment, db));
  ...
  if (req.method === "POST" && squareCommentMatch) {
    ...
    if (authorMode === "named" && !authorName) return sendError(res, 400, "请填写编辑昵称，或者选择匿名评论");
    ...
    return sendJson(res, 201, { comment: publicSquareComment(savedComment, db) });
  }

  const squareCommentLikeMatch = path.match(/^\/api\/square\/comments\/([^/]+)\/like$/);
  if (req.method === "POST" && squareCommentLikeMatch) {
    const comment = db.squareComments.find((item) => item.id === decodeURIComponent(squareCommentLikeMatch[1]));
    if (!comment) return sendError(res, 404, "没有找到这条评论");
    const body = await readJson(req);
    const voterId = cleanText(body.voterId, 80) || randomUUID();
    const existing = db.squareCommentLikes.find((like) => like.commentId === comment.id && like.voterId === voterId);
    if (!existing) {
      const like = {
        id: randomUUID(),
      ...
      if (useSupabase) await insertSquareCommentLike(like);
      else db.squareCommentLikes.push(like);
      if (useSupabase) clearStoreCache();
      if (!useSupabase) await saveDb(db);
    }
    const likeCount = existing ? db.squareCommentLikes.filter((like) => like.commentId === comment.id).length : db.squareCommentLikes.filter((like) => like.commentId === comment.id).length + (useSupabase ? 1 : 0);
    return sendJson(res, 200, { liked: true, likeCount });
  }
  ...
  if (req.method === "GET" && sentLettersMatch) {
    ...
    return sendJson(res, 200, { letters: ownerSentLetters(inbox, db) });
  }
  ...
  if (req.method === "GET" && ownerLettersMatch) {
    ...
    const letters = db.letters
      ...
      .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    return sendJson(res, 200, { inbox: publicInbox(inbox, db), letters, stats: ownerStats(letters), sent: ownerSentLetters(inbox, db), posts: ownerSquarePosts(inbox, db) });
  }
  ...
  if (req.method === "POST" && replyMatch) {
    ...
    if (useSupabase) {
      Object.assign(letter, await updateLetter(letter.id, patch));
      clearStoreCache();
    }
